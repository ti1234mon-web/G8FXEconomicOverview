...
st.markdown("""
    ...
    .indicator-bias-tag.bullish { 
        background: rgba(46, 204, 113, 0.15); 
        color: #2ecc71; 
        border-color: rgba(46, 204, 113, 0.3);
    }
    .indicator-bias-tag.rather-bullish { 
        background: rgba(46, 204, 113, 0.08); 
        color: #82e0aa; 
        border-color: rgba(46, 204, 113, 0.2);
    }
    .indicator-bias-tag.neutral { 
        background: rgba(212, 175, 55, 0.15); 
        color: #d4af37; 
        border-color: rgba(212, 175, 55, 0.3);
    }
    .indicator-bias-tag.rather-bearish { 
        background: rgba(231, 76, 60, 0.08); 
        color: #f5b7b1; 
        border-color: rgba(231, 76, 60, 0.2);
    }
    .indicator-bias-tag.bearish { 
...
# ==================== COLORED ARROW FUNCTIONS ====================
def get_colored_arrow(bias, size='small'):
    """Return HTML for colored arrow"""
    arrow_chars = {
        'Bullish': '▲',
        'Rather Bullish': '◢',
        'Neutral': '►',
    ...
    arrow_classes = {
        'Bullish': 'arrow-big-bullish' if size == 'big' else 'arrow-bullish',
        'Rather Bullish': 'arrow-big-rather-bullish' if size == 'big' else 'arrow-rather-bullish',
        'Neutral': 'arrow-big-neutral' if size == 'big' else 'arrow-neutral',
        'Rather Bearish': 'arrow-big-rather-bearish' if size == 'big' else 'arrow-rather-bearish',
        'Bearish': 'arrow-big-bearish' if size == 'big' else 'arrow-bearish'
    }
    
    char = arrow_chars.get(bias, '►')
    cls = arrow_classes.get(bias, 'arrow-neutral')
    
    return f'<span class="{cls}">{char}</span>'

def get_arrow_emoji(bias):
    """Fallback emoji arrows (kept for compatibility)"""
    arrows = {
        'Bullish': '⬆️',
        'Rather Bullish': '↗️',
        'Neutral': '➡️',
...
def main():
    ...
    with st.sidebar:
        ...
        <div style="background: #141414; padding: 15px; border-radius: 10px; border: 1px solid #1a1a0a;">
            <div style="display: flex; align-items: center; gap: 10px; padding: 3px 0;">
                <span style="color: #2ecc71; font-size: 1.3rem;">▲</span> <span style="color: #b0a080;">Bullish</span>
            </div>
            <div style="display: flex; align-items: center; gap: 10px; padding: 3px 0;">
                <span style="color: #82e0aa; font-size: 1.3rem;">◢</span> <span style="color: #b0a080;">Rather Bullish</span>
            </div>
            <div style="display: flex; align-items: center; gap: 10px; padding: 3px 0;">
                <span style="color: #d4af37; font-size: 1.3rem;">►</span> <span style="color: #b0a080;">Neutral</span>
            </div>
            <div style="display: flex; align-items: center; gap: 10px; padding: 3px 0;">
                <span style="color: #f5b7b1; font-size: 1.3rem;">◣</span> <span style="color: #b0a080;">Rather Bearish</span>
            </div>
            <div style="display: flex; align-items: center; gap: 10px; padding: 3px 0;">
                <span style="color: #e74c3c; font-size: 1.3rem;">▼</span> <span style="color: #b0a080;">Bearish</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Quick stats - count economies with bullish/bearish indicators
        total_indicators = 0
    ...
    with tab1:
        ...
        for idx, economy in enumerate(ECONOMIES):
            with cols[idx % 4]:
                ...
                arrow_html = get_colored_arrow(overall_bias, size='big')
                st.markdown(f"""
                <div class="summary-card">
                    <div class="summary-economy">{code}</div>
                    <div class="summary-currency-code">{name}</div>
                    <div style="font-size: 2rem; margin: 4px 0;">{arrow_html}</div>
        ...
        if selected_economy:
            ...
            # Track total indicators for this economy
                ...
                for category in CATEGORIES:
                    ...
                    if indicators:
                        ...
                        for idx, indicator in enumerate(indicators):
                            ...
                            arrow_html = get_colored_arrow(ind_bias, size='small')
                            ...
                            with cols[idx % 3]:
                                ...
                                <div class="indicator-item">
                                    ...
                                    <span>
                                        {arrow_html}
                                        <span class="indicator-bias-tag {get_bias_class(ind_bias)}">{ind_bias}</span>
                                    </span>
                                </div>
                                """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        # Economy selector for indicator management
        ...
        if mgmt_economy:
            ...
            if current_indicators:
                for idx, indicator in enumerate(current_indicators):
                    ...
                    with col2:
                        bias = indicator.get('bias', 'Neutral')
                        arrow_html = get_colored_arrow(bias, size='small')
                        st.markdown(f"{arrow_html} {bias}", unsafe_allow_html=True)
                    with col3:
                        if st.button("✏️", key=f"edit_{mgmt_economy}_{selected_category}_{idx}"):
                            st.session_state['editing_indicator'] = idx
                            st.session_state['editing_name'] = indicator.get('name', '')
                            st.session_state['editing_bias'] = indicator.get('bias', 'Neutral')
                    with col4:
                ...
                if 'editing_indicator' in st.session_state and st.session_state.get('editing_indicator') is not None:
                    idx = st.session_state['editing_indicator']
                    st.markdown("#### ✏️ Edit Indicator")
                    col1, col2 = st.columns(2)
                    with col1:
                        new_name = st.text_input("Indicator Name", value=st.session_state.get('editing_name', ''))
                    with col2:
                        new_bias = st.selectbox("Bias", BIAS_OPTIONS, index=BIAS_OPTIONS.index(st.session_state.get('editing_bias', 'Neutral')))
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("💾 Save Changes"):
                            current_indicators[idx]['name'] = new_name
                            current_indicators[idx]['bias'] = new_bias
                            indicators_data[mgmt_economy][selected_category] = current_indicators
                            data['indicators'] = indicators_data
                            save_data(data)
                            del st.session_state['editing_indicator']
                            del st.session_state['editing_name']
                            del st.session_state['editing_bias']
                            st.rerun()
                    with col2:
                        if st.button("❌ Cancel"):
                            del st.session_state['editing_indicator']
